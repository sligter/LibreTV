...
docker run -d \
  ...
  -p 8899:80 \
  -p 5000:5000 \
  -e PASSWORD=314159265h \
  -e DANMAKU_API_URL=http://111.119.239.46:5000 \
  bradleylzh/libretv:latest
```
...
LibreTV现已支持在线弹幕搜索和显示功能，支持以下视频平台的弹幕：
...
1. 在播放器右侧控制栏中，点击弹幕按钮(对话框图标)打开弹幕面板
2. 弹幕面板包含两个标签页：弹幕设置和弹幕搜索
3. 在弹幕设置中可以：
   - 开启/关闭弹幕显示
   - 调整弹幕透明度
   - 调整弹幕速度
...
1. 安装依赖：
```bash
pip install flask pandas aiohttp requests tqdm retrying beautifulsoup4 brotli google-api-python-client pyside6
```
...
本项目使用纯JavaScript开发，无需编译构建。

- `player.html` - 主播放器页面
- `app.py` - 弹幕服务后端
- `danmaku_loader.py` - 弹幕加载器核心类
- `get_*.py` - 各平台弹幕获取实现
